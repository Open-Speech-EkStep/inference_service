...
class ModelService:
    ...
    def __init__(self, model_config, decoder_type, cuda, half):
        ...
        for language_code, path in model_config.items():
            ...
            if language_code in ['en-IN', 'en-IN-BIO']:
                ...
                model_item.set_is_punctuation_and_itn_enabled(False)
    ...
    def apply_punctuation_and_itn(self, result, model_item, enable_punctuation, enable_inverse_text_normalization):
        if not model_item.is_punctuation_and_itn_enabled():
            return result
        language_code = model_item.get_language_code()
        if enable_inverse_text_normalization:
            language_code = 'en' if language_code == 'en-IN' or language_code == 'en-IN-BIO' else language_code
    ...
    def get_inference(self, file_name, language_code, enable_punctuation=False,
        ...
        result = self.apply_punctuation_and_itn(result, model_item, enable_punctuation,
    ...
    def get_srt(self, file_name, language_code, enable_punctuation=False, enable_inverse_text_normalization=False):
        ...
        result_arr = get_srt(file_name, model, generator, dict_file_path,
                             '/home/nireshkumarr/inference-wrapper/denoiser', audio_threshold=15,
                             language=language_code, half=self.half)
        srt_string = ''
        for result in result_arr:
            res = result[0]
            if result[3]:
                resp = self.apply_punctuation_and_itn(result[1], model_item, enable_punctuation,
                                                      enable_inverse_text_normalization)
...
if __name__ == "__main__":
    ...
    result = model_service.get_inference("/home/nireshkumarr/inference-wrapper/files/hindi/chunk_04.wav", 'hi', True,
                                         True)
